...
document.body.append(inputElement1, inputElement2, inputElement3);
...
inputElement1.addEventListener('click', function () {
    if (this.checked) {
        ...
        for (const element of status) {
            document.write(element.id, element.name, element.age, element.status, element.address.city, element.address.street,element.address.number)
...
inputElement2.addEventListener('click', function () {
    if (this.checked) {
        let status = (usersWithAddress.filter(value => value.age >= 29));
        for (const element of status) {
            document.write(element.id, element.name, element.age, element.status, element.address.city, element.address.street,element.address.number)
            document.write(`<div></div>`)
        }
...
inputElement3.addEventListener('click', function () {
    if (this.checked) {
        let status = (usersWithAddress.filter(value => value.address.city === 'Kyiv'));
        for (const element of status) {
            document.write(element.id, element.name, element.age, element.status, element.address.city, element.address.street,element.address.number)
            document.write(`<div></div>`)
        }
